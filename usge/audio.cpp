...
const int Audio::MAX_SOURCES = 200;
...
Audio::Audio()
{
    ...
    if(!alDevice)
    {
        cerr << "Audio::Audio() could not open audio device" << endl;
        exit(1);
    }
    ...
    if(!alContext)
    {
        cerr << "Audio::Audio() could not create audio context" << endl;
        exit(1);
    }
...
ALuint Audio::getSoundBuffer(std::string title)
{
    for (int i=0; i<soundBufferTitles.size(); i++) if (soundBufferTitles[i] == title) return soundBufferArray[i];
    return soundBufferArray[0];
}

ALuint Audio::loadWAV(string filename, string title)
{
    ifstream soundFile;
    ...
    {
        cerr << "Audio::loadWAV() could not load " << filename << "---does the file exist?" << endl;
        exit(1);
    }
    ...
    {
        cerr << "Audio::loadWAV() did not detect the RIFF/WAVE header combination in " << filename << endl;
        exit(1);
    }
    ...
    {
        cerr << "Audio::loadWAV() detected invalid wave format in " << filename << endl;
        exit(1);
    }
    ...
    {
        cerr << "Audio::loadWAV() detected invalid data header in " << filename << endl;
        exit(1);
    }
    ...
    {
        cerr << "Audio::loadWAV() could not load wave data in " << filename << " into struct" << endl;
        exit(1);
    }
    ...
    {
        cerr << "Audio::loadWAV() encountered AL error " << (int)error << " when buffering " << filename << endl;
        exit(1);
    }
    soundBufferArray.push_back(buffer);
    
    // then set the title to soundBufferTitles to searching al buffer from the name
    soundBufferTitles.push_back(title);
...
ALuint Audio::playSound(ALuint buffer)
{
...
ALuint Audio::playSound(ALuint buffer, glm::vec3 pos, double refDist, double maxDist)
{
...
ALuint Audio::loopSound(ALuint buffer)
{
    ALuint source = getUnusedSource();
...
ALuint Audio::loopSound(ALuint buffer, glm::vec3 pos, double refDist, double maxDist)
{
    ALuint source = getUnusedSource();
...
bool Audio::isSoundPlaying(ALuint source)
{
    int state;
    bool result = false;
...
void Audio::pause(ALuint source)
{
    int state;
...
void Audio::resume(ALuint source)
{
    int state;
...
void Audio::setListenerOrientation(glm::vec3 forward, glm::vec3 up)
{
