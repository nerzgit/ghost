...
#include "muzzleflashdecorator.hpp"
...
MuzzleFlashDecorator::MuzzleFlashDecorator(const char* obj_path,
                                           const char* tex_path)
{
    _firstrender = true;
...
MuzzleFlashDecorator::~MuzzleFlashDecorator()
{
...
/* In Rendering */
void MuzzleFlashDecorator::ProcessMuzzleFlash()
{
...
void MuzzleFlashDecorator::_ProcessInitializeObject(const char* obj_path,
                                                    const char* tex_path)
{
...
void MuzzleFlashDecorator::_ProcessRenderObject()
{
    ...
    if (_firstrender) {
        ...
        _valancematrix = glm::translate(glm::mat4(1.0), glm::vec3(0,stsfile.muzzleflashlengthy, stsfile.muzzleflashlengthz));
    ...
    if (_updatecounter == 0) {
        ...
        // ConstraintMatrix
        _constraintmatrix = WeaponDecorator::_o_weapon->RecieveConstraintMatrix(0) * _valancematrix;
    ...
    glUniformMatrix4fv(_synthesismatrixID, 1, GL_FALSE, &_constraintmatrix[0][0]);
...
void MuzzleFlashDecorator::UpdateMuzzleFlashConstraintMatrix(glm::mat4 &matrix){
    _constraintmatrix = matrix;
}

void MuzzleFlashDecorator::UpdateMuzzleFlashCounterToZero(){
    _updatecounter = 0;
}
